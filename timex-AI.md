# Timex: AI Writer Features

**AI Writer** is a collaborative AI writing assistant for Visual Studio Code. It allows you to "co-author" documents with an AI agent directly within your Markdown files using a simple, structured workflow. We have a special markdown comment syntax called `P-A-E Syntax` (described below), which lets you identify draft (human written) and final (AI written) sections in your markdown.

Instead of switching back and forth between a chat window and your document, AI Writer lets you define **Draft** sections and **AI** sections right in the text, creating a seamless "Human Draft -> AI Polish" loop.
...
### After creating the P-A-E section for the text selection about calculus shown in the image above, we get the following (see image below) when we run the **Generate from Draft** command with the edit cursor somewhere in the P-A-E section. Notice that the content after `<!-- a -->` (the AI content) has been filled in by the AI automatically. That is, the **Generate from Draft** command generated the "A-Section" of the content, by rewriting your own draft entirely by the AI.
...
AI Writer uses three specific HTML comment tags to structure your document:
...
2.  **Insert a Block**:
    *   Run the command `AI Writer: Insert Block Template` from the Command Palette (`Ctrl+Shift+P` or `Cmd+Shift+P`).
...
4.  **Invoke the Writer**:
    ...
    *   **Option A (Right-Click)**: Right-click in the editor and select **"AI Writer > Generate from Draft"**.
...
3.  Choose **"AI Writer > Generate from Draft"**.
...
2.  **Option A (Right-Click)**: Right-click in the editor and select **"AI Writer > Verify"**.
...
In addition to **Generate from Draft** (which rewrites your prose draft), AI Writer also supports **Generate from Outline**. This feature allows you to provide bullet points or a hierarchical outline in the P-Section, and the AI will expand them into fully written prose.

This is useful when you want to:
*   Quickly capture ideas as bullet points and let the AI flesh them out.
*   Create structured content from a hierarchical outline.
*   Generate paragraphs from a list of key points.

## How to Use

1.  Place your cursor inside a P-A-E block containing bullet points.
2.  **Option A (Right-Click)**: Right-click in the editor and select **"AI Writer > Generate from Outline"**.
...
Here's an example showing an outline being expanded into prose:

```markdown
<!-- p -->
Calculus Derivatives are everywhere in the real world:
- **Physics**
- **Economics**
- **Biology**
- **Engineering**
<!-- a -->
Derivatives play a crucial role in many fields, serving as a fundamental tool for 
understanding change. In physics, they help describe motion and forces. Economists use 
derivatives to analyze rates of change in markets and optimize strategies. In biology, 
derivatives assist in modeling population growth and reaction rates. Engineers rely on
derivatives to design systems and predict performance under varying conditions.
<!-- e -->
```

Notice how the simple bullet points in the P-Section were transformed into a cohesive paragraph in the A-Section. The AI understands the context and expands each point into meaningful content.

## Customizing the Outline Prompt

Similar to the draft prompt, you can override the outline prompt by creating a file named `AI-WRITER-GEN-FROM-OUTLINE.md` in your workspace root. If this file exists, AI Writer will use it instead of the built-in outline prompt.
...
2.  Select **AI Writer > Add File To Context**.
...
*   `AI Writer > Generate from Draft`: (Context Menu) Opens Chat and invokes `@writer` for the current block.
*   `AI Writer > Generate from Outline`: (Context Menu) Opens Chat and invokes `@writer /fill-outline` to generate from bullet points.
...
AI Writer adds a "AI Writer" submenu to the Explorer context menu, allowing you to manage P-A-E sections across your entire project.
