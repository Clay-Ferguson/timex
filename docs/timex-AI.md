...
### After creating the P-A-E section for the text selection about calculus shown in the image above, we get the following (see image below) when we run the **Gen. from Draft** command with the edit cursor somewhere in the P-A-E section. Notice that the content after `<!-- a -->` (the AI content) has been filled in by the AI automatically. That is, the **Gen. from Draft** command generated the "A-Section" of the content, by rewriting your own draft entirely by the AI.
...
4.  **Invoke the Writer**:
    ...
    *   **Option A (Right-Click)**: Right-click in the editor and select **Timex > "AI: Gen. from Draft"**.
...
3.  Choose **Timex > "AI: Gen. from Draft"**.
...
2.  **Option A (Right-Click)**: Right-click in the editor and select **Timex > "AI: Verify"**.
...
In addition to **Gen. from Draft** (which rewrites your prose draft), AI Writer also supports **Gen. from Outline**. This feature allows you to provide bullet points or a hierarchical outline in the P-Section, and the AI will expand them into fully written prose.
